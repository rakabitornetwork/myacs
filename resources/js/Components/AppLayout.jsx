...
export default function AppLayout({ children, title, actions }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-100">
      ...
      >
        <div className="flex h-11 shrink-0 items-center gap-2.5 border-b border-zinc-800/80 px-3.5">
          <Logo className="h-6 w-6 shrink-0" />
          <div className="min-w-0 flex-1">
            <p className="truncate text-[13px] font-semibold text-white">MyACS</p>
            <p className="truncate text-[11px] text-zinc-500">TeslaTech · TR-069</p>
          </div>
          <button
            type="button"
        ...
        <div className="shrink-0 border-t border-zinc-800/80 p-2.5">
          <div className="mb-2 rounded-md bg-zinc-900/80 px-2.5 py-2">
            <p className="truncate text-[13px] font-medium text-zinc-200">{auth?.user?.name}</p>
          ...
          <Link
            ...
            method="post"
            as="button"
            className="flex w-full items-center gap-2.5 rounded-md px-2.5 py-2 text-[13px] text-zinc-500 transition hover:bg-zinc-900 hover:text-zinc-200"
          >
      ...
      <div className="lg:pl-[13.5rem]">
        <header className="sticky top-0 z-30 flex h-11 items-center gap-2.5 border-b border-zinc-200/80 bg-white/90 px-3.5 backdrop-blur-md lg:px-4">
          <button
            type="button"
          ...
          <div className="ml-auto flex items-center gap-2">
            {app?.mode === 'dual' ? (
              ...
              <span className="hidden items-center gap-1 rounded-md bg-sky-500/10 px-1.5 py-0.5 text-[11px] font-medium text-sky-700 ring-1 ring-sky-500/20 sm:inline-flex">
                GenieACS Panel
              </span>
            ) : app?.cwmpEnabled ? (
              <span className="hidden items-center gap-1 rounded-md bg-emerald-500/10 px-1.5 py-0.5 text-[11px] font-medium text-emerald-700 ring-1 ring-emerald-500/20 sm:inline-flex">
